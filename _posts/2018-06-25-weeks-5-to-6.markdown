...
`seq_bellman_ford_shortest_paths(g, src, distmx)`: 39.675 s (92898 allocations: 764.61 MiB)

---

`parallel_bellman_ford_shortest_paths(g, src, distmx)`: 24.677 s (28290 allocations: 251.70 MiB)

---

I also attempted to parallelise it using atomic operations. However, the run-time shot
through the roof with this implementation
...
Although this created a significant speed up over the sequential version, the distributed implementations of centrality measures
performed better.