...
If you can recall the `parallel_page_rank` code I obtained a good performance improvement
 using `optimal_weighted_partition` to statically partition.
...
*Firstly*, BFS is a linear time algorithm. If partitioning `cur_level` at the beginning of 
each graph graph exploration phase takes alot of time then we will not obtain any speed up.
