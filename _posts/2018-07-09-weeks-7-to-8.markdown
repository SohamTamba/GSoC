...
I restricted the partitioning to contiguous partitions because the `num_part` partitions can be expressed in `num_part` integer ranges instead of `num_part` arrays that contain a total of `|V|` integers.
...
no difference but the latter is costlier.
...
`old_dijkstra_shortest_paths(g, src, distmx)`: 302.742 ms (522050 allocations: 47.43 MiB)

---

`new_dijkstra_shortest_paths(g, src, distmx)`: 265.332 ms (81397 allocations: 18.29 MiB)
