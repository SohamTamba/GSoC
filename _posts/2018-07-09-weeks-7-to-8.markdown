...
categories: GSoC
tags: GSoC
description: Degree based Graph Partitioning, Batch Priority Queue, Improved Prim, Improved Dijkstra
---
...
As explained in week 5 and 6, I found that Parallel Pagerank had an improvement in performance by
partitioning the vertices and assigning it to a processor in an attempt to balance the number
of edges assigned to each processor. To avoid race condition, no two processors could be assigned
edges that have the same destination vertex.

By balance, we mean to minimise that largest partition.

I restricted the partitioning to contiguous partitions because the `p` partitions can be expressed in `p` integer ranges instead of `p` arrays that contain a total of `|V|` integers.
...
function greedy_partition(degree_list::Vector{Integer}, num_part::Int)
    ...
    for part_remain in reverse(1:(num_part-1))
        ...
        part_size = 0
        current_balance = max(pat_size, suffix_sum[right+1]/part_remain)
...
`up_bound` can be set to `sum(degree_list)` and `low_bound` can be set to 0.
...
Using the optimal algorithm over the greedy algorithm for page rank on Ego Twitter Graph resulted in a minor increase in run-time. (65ms to 73ms)
...
`new_prim(g, distmx)`:  228.200 ms (802705 allocations: 20.28 MiB) 
...
`distmx` of type `SparseMatrixCSC{Float64,Int64}`: 527.681 ms (292673 allocations: 32.86 MiB)
...
### II. Collecting additional data.

Dijkstra has some optional variables to collect more data than `dists` (and parents). For example, 
...
I replaced
{% highlight julia %}
function dijkstra(g::AbstractGraph, source, distmx::AbstractMatrix)  # allpaths = true
    dists = fill(typemax(eltype(g)), nv(g))
    dists[source] = 0
...
`old_dijkstra_shortest_paths`: 302.742 ms (522050 allocations: 47.43 MiB)

---

`new_dijkstra_shortest_paths`: 265.332 ms (81397 allocations: 18.29 MiB)
