...
**Justification:** The machine level instructions of write are idempotent. 
...
As you can see, it explored the graph starting from the `sources`.
...
function parallel_gdistances!(
    ...
    while !isempty(cur_level)
        ...
        @threads for v_set in partitions
            t = threadid()
            @inbounds next_level = next_level_t[t] #Each thread gets its own next_level array
            
            @inbounds for v in v_set
...
The idea is, each thread `t` will be provided its own `cur_level` array (`cur_level_t[t]`) as well as its own `next_level` array (`next_level_t[t]`). During each iteration of graph exploration,
...
vertices from other `cur_level` arrays and explore them.
